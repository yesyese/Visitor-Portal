...
export const authAPI = {
  ...
  forgotPassword: async (email) => {
    return apiCall('/foreign-users/forgot-password', {
      method: 'POST',
  ...
  logout: async () => {
    try {
      ...
      // Still return success to allow the logout to proceed
      return { success: true };
    }
  },
...
export const apiUtils = {
  ...
  removeAuthToken: () => {
    ...
    // Remove auth header from API config
    delete apiConfig.headers.Authorization;
  },
  isAuthenticated: () => {
    // Check both localStorage and API headers to be extra sure
    const hasLocalToken = !!localStorage.getItem('userToken');
    const hasApiToken = !!apiConfig.headers.Authorization;
    
    // If localStorage has token but API config doesn't, sync them
    if (hasLocalToken && !hasApiToken) {
      const token = localStorage.getItem('userToken');
      apiConfig.headers.Authorization = `Bearer ${token}`;
    }
    
    return hasLocalToken || hasApiToken;
  },
  getToken: () => {
